...
<template>
  <teleport to="body">
    <dialog v-if="model" class="base-modal" ref="dialogRef" @click="onClick" @cancel="$emit('onCancel')">
      ...
      <main class="base-modal-body">
        <slot name="body" :formId="formId" :closeModal="closeModal" />
      </main>
      <footer class="base-modal-footer">
        <slot name="footer" :formId="formId" :closeModal="closeModal" />
      </footer>
    </dialog>
  </teleport>
</template>
